...
class NeMoModelCheckpoint(ModelCheckpoint):
    ...
    def _backup_existing_nemo_ckpt(self, trainer) -> Optional[str]:
        ...
        if trainer.is_global_zero:
            last_model_path = next(
                (cb.last_model_path for cb in trainer.callbacks if isinstance(cb, NeMoModelCheckpoint)), None)
            last_global_step_saved = re.findall(r'step=(\d+)', last_model_path)[0]
            last_global_epoch_saved = re.findall(r'epoch=(\d+)', last_model_path)[0]
            available_path = re.sub(r'(.*)(v\d+)', r'\1step_{}_epoch_{}'.format(last_global_step_saved, last_global_epoch_saved), available_path)
